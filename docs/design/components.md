...
| `Transport` | play / pause / stop with state visible without colour | **`state`**, **`onPlay`**, **`onPause`**, **`onStop`**, `beatPulseKey`, `canPlay` `true`, `canPause` `true`, `disabledReason`, `labels`, `size` `'lg'`, `playRef` | ours + Button | M1-07 |
