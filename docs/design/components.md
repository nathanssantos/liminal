...
| `Transport` | play / pause / stop with state visible without colour | **`state`**, **`onPlay`**, **`onPause`**, **`onStop`**, `beatPulseKey`, `canPlay` `true`, `canPause` `true`, `stopOnly` `false` (one button that reads Play or Stop, for a transport with no pause), `disabledReason`, `labels`, `size` `'lg'`, `playRef` | ours + Button | M1-07 |
