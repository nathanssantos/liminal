...
| 2026-09-06 | M1-02 | @liminal/engine: nine presets, filter and eq3, lookahead scheduling over Tone's transport, automation per curve, a disposal ledger, and play:fixture for the listening gate | Tone's transport PPQ is 192 against the score's 960; a Tone.Context around an offline context never advances the transport |
| 2026-09-06 | M1-02 | @liminal/engine: nine presets, filter and eq3, lookahead scheduling over Tone's transport, automation per curve, a disposal ledger, and play:fixture for the listening gate | review found the bar number and the automation both taken from the context clock, which only agrees with the transport in an offline render; the suite went from 47 s to 8 s once tests rendered only what they observe |
