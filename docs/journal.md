...
| 2026-09-06 | M1-02 | @liminal/engine: nine presets, filter and eq3, lookahead scheduling over Tone's transport, automation per curve, a disposal ledger, and play:fixture for the listening gate | review found the bar number and the automation both taken from the context clock, which only agrees with the transport in an offline render; the listening gate then found the transport stopping on the last tick, which rewinds the tick source mid-tick and makes every part replay its first event — 165 attacks on a document holding 48; the suite went from 47 s to 8 s once tests rendered only what they observe |
| 2026-09-06 | M1-06 | `board.review`: the prepared worktree, the scratch copy, the review state, the findings writer, and the merge gate that reads them; `board.check --path` | six rounds found six ways to a green merge with no review — an empty measured set, the local checkout's branch and head, a swapped pair, an empty branch, a card-less branch refused, and findings nothing could write. None showed up in the tests; all came from driving the tool as an adversary |
| 2026-09-06 | M1-03 | renderOffline, encodeWav and measureBasic: the fixture renders identically twice, the seed does not move a sample, and the wav header carries the duration | the < 5 s criterion failed on the machine it names; the first rewrite derived a weaker bound from the wrong constant, and the honest reading is that Node misses `SOUNDCHECK_BUDGET` by ~35% and Chromium is the gate (M1-05) |
