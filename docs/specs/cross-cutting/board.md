...
| `board.deliver` | `--gates-only` \| `--rebase-only` \| `--open --title --description [--dry-run]` \| `--ready <pr>` \| `--merge <pr>` (reads `review.json`: refuses with an open blocking finding, without the deep pass on the final head, or when the branch names an issue no spec claims; a branch with no issue number carries no review to check and passes) \| `--stale-description` \| `--stale-docs` | `verdict` and each gate's detail; never truncates command output (saves it to `evidence/_gates/<date>.log` and cites the path) |
