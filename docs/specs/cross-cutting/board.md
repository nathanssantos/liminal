...
| `board.check` | `--area <path>` \| `--path <root>` | `newComments[]` · `deadMocks[]` · `deadBranches[]` · `outOfArea[]` · `testsWithoutCriterion[]` · `dirtyTree` |
| `board.deliver` | `--gates-only` \| `--rebase-only` \| `--open --title --description [--dry-run]` \| `--ready <pr>` \| `--merge <pr>` (asks GitHub for the pull request's branch and head, then reads `review.json`: refuses with an open blocking finding, without the deep pass on that head, when the deep pass recorded nothing measured, when the diff since it cannot be read, or when the branch names an issue no spec claims; a branch with no issue number carries no review to check and passes) \| `--stale-description` \| `--stale-docs` | `verdict` and each gate's detail; never truncates command output (saves it to `evidence/_gates/<date>.log` and cites the path) |
| `board.review` | `--prepare` \| `--scratch` \| `--state` \| `--round-done` | `--prepare`: one detached worktree at HEAD under `/tmp/liminal-review/<branch>/<sha>/` with `pnpm install` done (from the store, offline when possible), reused across agents and rounds while the head is the same; returns `reviewPath`, `head`, `reviewedHead`, `round`. `--scratch`: a throwaway worktree at the same commit, with its own offline install, for an agent that edits or reverts; returns its path; `--clean` removes them. `--findings <file>`: records the round's findings from a JSON array, refusing a `severity` outside blocking/major/minor, a `status` outside open/fixed/discarded/accepted, or a missing `agent`, `round` or `summary` — the three fields the gate reads — and replacing only the entries of the same `agent` and `round` so agents in one round accumulate. `--state`: `evidence/<id>/review.json` — `round`, `head`, `reviewedHead`, `deepPassHead`, `findings[]` (`agent`, `severity`, `file`, `line`, `summary`, `status`: open/fixed/discarded/accepted, `round`). `--round-done`: records the reviewed head and, with `--deep`, the deep pass head |
